function varargout = SLMPhaseMaskMaker3D_OpenCL(varargin)
% Lloyd Russell 2015-2019
% Implements a weighted GS algorithm using Meadowlark's generate_hologram 
% dll shipped with Bruker PrairieView.
...
% |----- (filename)_OpenCLphase.ext   < the computed phase mask
...
for f = 1:NumFiles
    ...
    % shift targets based on offset to correct slight alignment errors
    if Do2DTransform || Do3DTransform
        x_i = x_i - yaml.SLM_Offset_X;
        y_i = y_i - yaml.SLM_Offset_Y;
        if Do3DTransform
            z_i = z_i - yaml.SLM_Offset_Z;
        end
    end
...
%         keyboard
    ...
    pSLMstart       = zeros(SLMsize) + 0.5;  % the starting phase mask
    pSLM            = zeros(SLMsize);
    ...
    if p.Results.Save
        ...
        if isempty(strfind(SaveName{f},'.tiff')) && ~isempty(strfind(SaveName{f},'.tif'))
            imwrite(PhaseMasks{f}, ['PhaseMasks' filesep strrep(SaveName{f}, '.tif', ['_OpenCLphase.' p.Results.SaveFormat])]);
        elseif ~isempty(strfind(SaveName{f},'.tiff'))
            imwrite(PhaseMasks{f}, ['PhaseMasks' filesep strrep(SaveName{f}, '.tiff', ['_OpenCLphase.' p.Results.SaveFormat])]);
        else
