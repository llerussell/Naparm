...
p.addParameter('Do3DTransform', 0); % was true 
...
p.addParameter('Iterations', 1); % was 100, had to change it for making large phase masks, can be set higher for up to 1024x1024 
...
if AutoAdjustWeights
    load(['C:\Users\User\Dropbox\Bruker2\SLM\Weighting\' yaml.WeightingFile], 'W');  % loads 'W'
end
...
%         keyboard
    ...
    if AutoAdjustWeights
        distances = pairwiseDistance([x y], [256 256]); % is this correct? may have to be center of SLM zero order which is 700 550 or sth 
    ...
    % added Dustin, to avoid errors in making large masks for large rectangluar
    % SLM --> compared 1 and 100 iterations manually and did not see
    % massive difference in first order power 
