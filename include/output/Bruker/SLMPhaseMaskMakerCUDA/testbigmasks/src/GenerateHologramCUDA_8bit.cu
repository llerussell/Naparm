...
#endif
#define MAX_SPOTS 100	//decrease this if your GPU keeps running out of memory, was 1024
#define BLOCK_SIZE 512	//should be a power of 2, was 512 
...
__global__ void ApplyCorrections(uint8_t *g_pSLM_uc, uint8_t *g_LUT, float *d_AberrationCorr_f, float *d_LUTPolCoeff_f);
__global__ void LensesAndPrisms(uint8_t *g_SLMuc, uint8_t *g_LUT, float *d_AberrationCorr_f, float *d_LUTPolCoeff_f);
__global__ void calculateIobtained(uint8_t *g_pSLM_uc, float *g_Iobtained);
__global__ void PropagateToSLM_Fresnel(float *g_spotRe_f, float *g_spotIm_f, float *g_pSLM2pi, float *g_weights, int iteration, float *g_pSLMstart, float *g_amps,
										bool getpSLM255, uint8_t *g_pSLM255_uc, uint8_t *g_LUT, float *g_AberrationCorr_f, float *g_LUTPolCoeff_f);
...
__global__ void ReplaceAmpsSLM_FFT(float *g_aLaser, cufftComplex *g_cAmp, float *g_pSLMstart, bool getpSLM255, uint8_t *g_pSLM255_uc, uint8_t *g_LUT, float *g_AberrationCorr_f, float *g_LUTPolCoeff_f);
...
__global__ void f2uc(uint8_t *uc, float *f, int N_pixels, uint8_t *g_LUT, int use_linLUT, int data_w);
__global__ void uc2f(float *f, uint8_t *uc, int N);
...
inline void mDisplayDataUC(uint8_t *d_data, int length, int line);
...
uint8_t *d_pSLM_uc;						//The optimized pSpot pattern, uint8_t, the one sent to the SLM [0, 255]
uint8_t *h_LUT_uc;
uint8_t *d_LUT_uc = NULL;
...
//Generate a hologram
extern "C" __declspec(dllexport)  int GenerateHologram(float *h_checkData, uint8_t *h_pSLM_uc, float *x_spots, float *y_spots, float *z_spots, float *I_spots, int N_spots, int N_iterations, float *h_Iobtained, int method)//, float* gpuTime)
{
	//*gpuTime = 0;
	//float deltaTime = 0;
...
extern "C" __declspec(dllexport)  int startCUDA(float *h_pSLMstart, int deviceId)
{
    ...
    memsize_SLMuc = N_pixels * sizeof(uint8_t);
...
__device__ uint8_t phase2uc(float phase2pi)
{
	return (uint8_t)floor((phase2pi + M_PI)*65536.0f / (2.0f * M_PI));
}
...
__global__ void ApplyCorrections(uint8_t *g_pSLM_uc, uint8_t *g_LUT, float *g_AberrationCorr_f, float *g_LUTPolCoeff_f)
{
...
__global__ void calculateIandPhase(uint8_t *g_pSLM_uc, float *g_Iobtained, float *g_Pobtained)
{
	int blockSize = c_data_w[0];
	int spot_number = blockIdx.x;
	int tid = threadIdx.x;
...
__global__ void PropagateToSLM_Fresnel(float *g_spotRe_f, float *g_spotIm_f, float *g_pSLM2pi, float *g_weights, int iteration, float *g_pSLMstart, float *g_Iobtained, bool getpSLM255, uint8_t *g_pSLM255_uc,
								uint8_t *g_LUT, float *g_AberrationCorr_f, float *g_LUTPolCoeff_f)
{
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	int tid = threadIdx.x;
...
__global__ void ReplaceAmpsSLM_FFT(float *g_aLaser, cufftComplex *g_cAmp, float *g_pSLMstart, bool getpSLM255, uint8_t *g_pSLM255_uc,
								uint8_t *g_LUT, float *g_AberrationCorr_f, float *g_LUTPolCoeff_f)
{
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	if (idx<c_N_pixels[0])
...
//Convert from uint8_t [0, 255] to float [-pi, pi]
__global__ void uc2f(float *f, uint8_t *uc, int N)
{
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	if (idx<N)
...
inline void mDisplayDataUC(uint8_t *d_data, int length, int line)
{
	...
	do
	{
		...
		length = (length<=maxlength) ? length : maxlength;
		char MessageString[1000];
		h_data = (uint8_t*)malloc(length * sizeof (uint8_t));
		M_SAFE_CALL(cudaMemcpy(h_data, d_data, length*sizeof(uint8_t), cudaMemcpyDeviceToHost));
		sprintf(MessageString,	"Line: %d\nData: ", line);
		for (int ii = 0;ii<length;++ii)
		{
...
extern "C" __declspec(dllexport)  int GetIandPhase(float *x_spots, float *y_spots, float *z_spots, float *h_pSLM_uc, int N_spots_all, int data_w, float *h_I_obt, float *h_Phase_obt)
{
	...
	while (N_spots_rem > 0)
	{
		...
		//calculateIobtained(uint8_t *g_pSLM_uc, float *g_Iobtained)
		cudaDeviceSynchronize();
